...
def loop_reasoning_call_kwargs(reasoning_effort: str | None, model: str) -> dict:
    ...
    only form that works for a zero-data-retention organization. Every other
    route already returns what replay needs, so its kwargs are unchanged.
    ...
    if reasoning_effort is not None and "/responses/" in model:
