...
every version, which is why the range was originally mis-diagnosed as `<1.92`.

This test pins the CONSTRAINT, not the runtime: it asserts the resolved litellm
version is outside the broken range. The runtime repro requires network + a clean
venv per version and does not belong in unit CI.
"""
